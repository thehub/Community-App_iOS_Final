...
class ProjectViewController: ListFullBleedViewController {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        if let vm = data[indexPath.item] as? ProjectObjectiveViewModel {
            let cellWidth: CGFloat = self.collectionView.frame.width
            let height = vm.objective.description.height(withConstrainedWidth: cellWidth - 135 - 20, font:UIFont(name: "GTWalsheim-Light", size: 12.5)!) + 100 // add extra height for the standard elements, titles, lines, sapcing etc.
            return CGSize(width: view.frame.width, height: height)
        }
...
//            if project.groupType == .public {
//                return true
//            }
//            if !MyGroupsManager.shared.isInGroup(groupId: project.chatterId) {
//                let alertController: UIAlertController = UIAlertController(title: nil, message: nil, preferredStyle: .actionSheet)
//                let cancelAction: UIAlertAction = UIAlertAction(title: "Cancel", style: .cancel) { action -> Void in
//                    //cancel code
//                }
//                let joinAction: UIAlertAction = UIAlertAction(title: "Request To Join Project", style: .destructive) { action -> Void in
//                    UIApplication.shared.isNetworkActivityIndicatorVisible = true
//                    firstly {
//                        APIClient.shared.joinGroup(groupId: self.project.chatterId)
//                        }.then { result -> Void in
//                            print(result)
//                        }.always {
//                            UIApplication.shared.isNetworkActivityIndicatorVisible = false
//                        }.catch { error in
//                            debugPrint(error.localizedDescription)
//                            let alert = UIAlertController(title: "Error", message: "Could not request to join project. Please try again.", preferredStyle: .alert)
