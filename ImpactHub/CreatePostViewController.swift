...
//                })
                ...
                case .comment(let postIdToCommentOn, let toUserId):
                    firstly {
                        APIClient.shared.postComment(newsID: postIdToCommentOn, message: text)
                            ...
                            _ = APIClient.shared.sendPush(fromUserId: SessionManager.shared.me?.member.userId ?? "", toUserIds: toUserId, pushType: .comment(id: postIdToCommentOn, feedElementId: postIdToCommentOn, chatterGroupId: ""), relatedId: comment.id)
                        }.then { result in
                            self.onClose(self)
                        }.always {
                            self.inTransit = false
                ...
                case .contactRequest(let contactId):
                    firstly {
                        APIClient.shared.createDMRequest(fromContactId: SessionManager.shared.me?.member.contactId ?? "", toContactId: contactId, message: self.textView.text)
                        }.then { result -> Void in
                            if #available(iOS 10.0, *) {
                                self.generatorNotification.notificationOccurred(.success)
                            }
